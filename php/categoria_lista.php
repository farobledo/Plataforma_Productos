<?php
        ...
        <table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
            <thead>
                <tr class="has-text-centered">
                    ...
                    <th class="table-success">Ubicación</th>
                    <th class="table-primary">Productos</th>
                    <th class="table-info" colspan="2">Opciones</th>
                </tr>
            </thead>
            <tbody>
	';
	...
	if($total>=1 && $pagina<=$Npaginas){
		...
		foreach($datos as $rows){
			$tabla.='
				<tr class="has-text-centered" >
					<td class="table-danger">'.$contador.'</td>
                    <td class="table-info">'.$rows['categoria_nombre'].'</td>
                    <td class="table-info">'.substr($rows['categoria_ubicacion'],0,25).'</td>
                    <td>
